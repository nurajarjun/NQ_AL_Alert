...
class ContextAnalyzer:
    ...
    async def _get_recent_news(self) -> List[Dict]:
        ...
        try:
            ...
            from_time = (datetime.now() - timedelta(hours=2)).isoformat()
            
            async with aiohttp.ClientSession() as session:
                url = "https://newsapi.org/v2/everything"
                params = {
                    "apiKey": self.news_api_key,
                    "q": "nasdaq OR tech stocks OR futures",
                    "from": from_time,
                    "sortBy": "publishedAt",
                    "language": "en",
                    "pageSize": 5
    ...
    async def _get_market_conditions(self) -> Dict:
        """Get current market conditions (VIX, SPY trend, etc.)"""
        try:
                    ...
                    if response.status == 200:
                        ...
                        if quote:
                            ...
                            return {
                                ...
                                "volatility_estimate": "High" if abs(change_pct) > 1 else "Normal"
                            }
        except Exception as e:
    ...
    def _analyze_time_of_day(self) -> Dict:
        ...
        # Best times: 9:30-11:30 AM, 2:00-3:30 PM
        # Avoid: 11:30-2:00 PM (lunch chop), after 3:30 PM (close)
        
        if 9.5 <= time_decimal <= 11.5:
            quality = "Excellent"
            reason = "Morning session - high liquidity and trends"
        elif 14 <= time_decimal <= 15.5:
            quality = "Good"
            reason = "Afternoon session - decent liquidity"
        elif 11.5 <= time_decimal <= 14:
            quality = "Poor"
            reason = "Lunch hour - choppy, low volume"
        elif time_decimal > 15.5:
            quality = "Risky"
            reason = "Near close - unpredictable moves"
        else:
            quality = "Pre-market"
        ...
        return {
            ...
            "reason": reason,
            "is_prime_time": quality in ["Excellent", "Good"]
        }
