...
class AIAnalyzer:
    ...
    async def analyze_trade(
        ...
        try:
            ...
            expert_context = ExpertContext().get_context_string()
            
            # Append expert context to prompt
            full_context = context_data or {}
            if expert_context:
                # If modifying prompt string directly isn't possible here without changin prompt templates, 
                # we'll pass it as part of context_data or wrap the prompt generation
                pass
            ...
            # INJECT CROSS-MARKET CONTEXT FOR NQ
            if signal_data.get('symbol', 'NQ') == 'NQ':
                nq_context = ExpertContext().get_nq_context()
                if nq_context:
                    prompt += f"\n\nCROSS-MARKET INTEL (SPX -> NQ):\n"
                    prompt += f"- Implied Regime: {nq_context['implied_regime']}\n"
                    prompt += f"- Strategy: {nq_context['implied_strategy']}\n"
                    prompt += f"- Note: {nq_context['note']}\n"
    ...
    def _validate_analysis(self, analysis: Dict, signal_data: Dict) -> Dict:
        ...
        # --- EXPERT PLAN SCORE ADJUSTMENT ---
        try:
            from analysis.expert_input import ExpertContext
            expert = ExpertContext()
            magnet = expert.get_magnet_level()
            
            if magnet and 'entry' in signal_data:
                entry = signal_data['entry']
                direction = signal_data['direction']
                
                # Boost MEAN REVERSION trades
                # If Price > Magnet and Signal is SHORT -> Good
                if entry > magnet and direction == 'SHORT':
                    analysis['score'] += 10
                    analysis['reasoning'].append(f"Aligned with Expert Magnet ({magnet})")
                
                # If Price < Magnet and Signal is LONG -> Good
                elif entry < magnet and direction == 'LONG':
                    analysis['score'] += 10
                    analysis['reasoning'].append(f"Aligned with Expert Magnet ({magnet})")
                    
                # Penalize TRENDING trades away from magnet
                elif (entry > magnet and direction == 'LONG') or (entry < magnet and direction == 'SHORT'):
                    analysis['score'] -= 10
                    analysis['reasoning'].append(f"Fighting Expert Magnet ({magnet})")
                    
        except Exception as e:
            logger.warning(f"Expert adjustment failed: {e}")
        # ------------------------------------
    ...
    def should_send_alert(self, analysis: Dict) -> bool:
        ...
        # Stricter override: Only allow "YES" with small margin (5 points)
        if recommendation == 'YES' and score >= (threshold - 5):
             return True
