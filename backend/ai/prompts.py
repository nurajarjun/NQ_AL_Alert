    ...
    def get_trade_analysis_prompt(signal_data: dict, context_data: dict) -> str:
        ...
        if news:
            ...
            for i, article in enumerate(news[:3], 1):
                news_summary += f"{i}. {article['title']} ({article['sentiment']})\n"
        else:
...
Time of Day: {time_info.get('current_time', 'Unknown')} - {time_info.get('time_quality', 'Unknown')}
...
4. Consider: R/R ratio, market context, time of day, news sentiment
5. Be conservative - it's better to skip than force a bad trade
6. Focus on risk management and capital preservation
