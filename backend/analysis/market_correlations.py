...
class MarketCorrelations:
    ...
    def analyze_correlations(self) -> dict:
        ...
        try:
            ...
            # Get DXY (Dollar Index - typically inverse to NQ)
            dxy_data = self._get_dxy_analysis()
            correlations['dxy'] = dxy_data
            
            # Get Bitcoin (BTC leads tech often)
            btc_data = self._get_btc_analysis()
            correlations['btc'] = btc_data
    ...
    def _get_btc_analysis(self) -> dict:
        """
        Analyze Bitcoin (BTC-USD)
        Correlation: High (Risk-on asset like NQ)
        """
        try:
            ...
            if change_pct > 2:
                signal = 'BULLISH'
                note = 'BTC Rallies -> Risk On'
            elif change_pct < -2:
                signal = 'BEARISH'
                note = 'BTC Dumping -> Risk Off'
            else:
                signal = 'NEUTRAL'
                note = 'BTC Stable'
    ...
    def _get_dxy_analysis(self) -> dict:
        """Analyze Dollar Index (Inverse to Assets)"""
        try:
            dxy = yf.Ticker('DX-Y.NYB')
            hist = dxy.history(period='5d')
            if hist.empty: return self._fallback_dxy()
            current = hist['Close'].iloc[-1]
            prev = hist['Close'].iloc[-2]
            change_pct = ((current - prev)/prev)*100
            
            return {'price': float(current), 'change_pct': float(change_pct)}
        except: return self._fallback_dxy()
        
    def _fallback_btc(self):
        return {'price': 0, 'change_pct': 0, 'signal': 'NEUTRAL', 'note': 'Data unavailable'}
        
    def _fallback_dxy(self):
